...
const ServiceList = () => {
    ...
    const fetchServices = async () => {
      try {
        ...
        const uniqueCategories = [
          ...new Set(data.map((service) => service.profession.category.title)),
        ];
        setCategories(uniqueCategories);

        const uniqueRegions = [
          ...new Set(
            data.map((service) => service.profession.user.gouvernorat)
          ),
        ];
  ...
  const filteredServices = services.filter((service) => {
    return (
      (selectedCategory === "" ||
        service.profession.category.title === selectedCategory) &&
      (selectedRegion === "" ||
        service.profession.user.gouvernorat === selectedRegion)
    );
  });
  ...
  const handleViewMore = (service) => {
    const relatedServices = services.filter(
      (s) =>
        s.profession.user.id === service.profession.user.id &&
        s.id !== service.id
    );
    navigate(`/service/${service.id}`, {
      state: { service, relatedServices, allServices: services },
    });
  };
    ...
    <div className="bg-white shadow-md rounded-lg p-6 pt-8 pb-8 mb-4 font-sans">
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-xl font-medium font-sans mb-4 text-gray-600">
          Top Rated Services of the Month
        </h2>
